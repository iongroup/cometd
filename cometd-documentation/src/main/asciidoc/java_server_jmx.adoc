...
Server-side CometD components may be exposed as JMX MBeans and visible via monitoring tools such as https://adoptium.net/jmc/[Java Mission Control], JVisualVM or JConsole.
...
[NOTE]
====
The context attribute names can be configured, see xref:_java_server_configuration[this section] for `BayeuxServer`, xref:_java_oort_common_configuration[this section] for `Oort, and xref:_java_oort_seti[this section] for `Seti`.
====
