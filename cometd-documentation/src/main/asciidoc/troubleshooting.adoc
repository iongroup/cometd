...
You can therefore plug in any logging implementation that supports SLF4J, and configure the logging category `org.cometd` at `INFO` level.
...
By default, SLF4J comes with a simple binding that _does not_ log statements produced at debug level.

Therefore, you must configure SLF4J with a more advanced binding such as `jetty-slf4j-impl`, https://logging.apache.org/log4j/2.x/[Log4J2] or https://logback.qos.ch/[Logback].
